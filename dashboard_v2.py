...
def save_matches_data(matches, filename="matches.csv"):
    try:
        ...
        # Sidebar filters for notable match criteria
        st.sidebar.header("Notable Match Criteria")
        notable_events = st.sidebar.multiselect(
            "Select Notable Events",
            options=['US Open', 'Australian Open', 'French Open', 'Wimbledon'],
            default=['US Open', 'Australian Open', 'French Open', 'Wimbledon']
        )
        high_margin_threshold = st.sidebar.slider(
            "High Margin Threshold (Games)",
            min_value=5,
            max_value=20,
            value=10
        )
...
def identify_notable_matches(row):
    # Example criteria
    notable_events = ['US Open', 'Australian Open', 'French Open', 'Wimbledon']
    high_margin_threshold = 10  # Adjust based on your data
